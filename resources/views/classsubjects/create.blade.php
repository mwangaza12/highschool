...
@section('content')
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('classsubjects.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
            ...
            <div>
                ...
                <input type="text" name="subject_id" id="subject_id" class="block w-full mt-1 rounded-md shadow-sm border-gray-300 focus:border-indigo-500 focus:ring focus:ring-indigo-200 focus:ring-opacity-50" value="{{ old('subject_id') }}">
                @error('subject_id')
                    <p class="text-sm text-red-500 mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div>
                <button type="submit" class="bg-blue-500 text-white px-4 py-2 rounded hover:bg-blue-600 focus:outline-none focus:bg-blue-600">Add Subject</button>
            </div>
        </form>
    </div>
@endsection
